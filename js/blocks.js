...
const Blocks = {
    ...
    // Paste image from clipboard into an image block URL field
    handleImageInputPaste(e, blockIndex, imageIndex) {
            ...
            if (file.type.startsWith('image/')) {
                ...
                reader.onload = (ev) => {
                    ...
                    if (imageIndex !== undefined && imageIndex !== null) {
                        this.updImg(blockIndex, imageIndex, 'src', dataUrl);
                    } else {
                        this.upd(blockIndex, 'src', dataUrl);
                    }
                    this.render();
                    Preview.render();
                };
        ...
        // Also check for image items (Chrome)
    ...
    ctrlImage(block, i) {
        ...
        <div class="ctrl"><label>Image</label>
            ...
            <input type="text" value="${block.src}" placeholder="Or paste image URL (Ctrl+V)" oninput="Blocks.upd(${i},'src',this.value)" onpaste="Blocks.handleImageInputPaste(event,${i},null)" style="margin-top:6px;">
        </div>
        ...
        <div class="row2">
            ...
            <div class="ctrl"><label>Height (px)</label>
                <input type="number" value="${block.height||''}" min="0" max="800" placeholder="auto"
                    oninput="Blocks.upd(${i},'height',parseInt(this.value)||0)">
            </div>
        </div>
        ...
        const imgPanel = (img, idx) => {
            ...
            <div class="img2-panel">
                ...
                <div class="ctrl" style="margin-bottom:4px;">
                    ...
                    <input type="text" value="${img.src}" placeholder="Or paste URL / Ctrl+V image"
                        oninput="Blocks.updImg(${i},${idx},'src',this.value)"
                ...
                <div class="row2" style="gap:6px;margin-bottom:4px;">
                    <div class="ctrl" style="margin:0;"><label>W (px)</label>
                        <input type="number" value="${img.width||280}" min="20" max="600"
                            oninput="Blocks.updImg(${i},${idx},'width',parseInt(this.value)||280)">
                    </div>
                    <div class="ctrl" style="margin:0;"><label>H (px)</label>
                        <input type="number" value="${img.height||''}" min="0" max="600" placeholder="auto"
                            oninput="Blocks.updImg(${i},${idx},'height',parseInt(this.value)||0)">
                    </div>
    ...
    uploadImg2(bi, idx) {
        Utils.handleImageUpload(data => {
            const blocks = State.get().blocks;
            if (blocks[bi]?.images?.[idx]) {
                blocks[bi].images[idx].src = data;
                State.updateBlocks(blocks); this.render(); Preview.render();
