...
const UI = {
    ...
    buildHeaderPanel() {
        ...
        el.innerHTML = `
            ...
            <div id="h-settings" style="${!enabled ? 'opacity:0.4;pointer-events:none;' : ''}">
                <div class="ctrl">
                    ...
                    <div class="img-action-row" style="margin-top:5px;">
                        <button type="button" class="clipboard-paste-btn" onclick="UI.pasteHeaderLogoClipboard()">
                            <i class="fas fa-clipboard"></i> Paste from Clipboard
                        </button>
                        <button type="button" class="browse-lib-btn" onclick="UI.browseLibForLogo()">
                    ...
                    <div class="img-url-row">
                        <input type="text" id="h-logo-url" placeholder="Or paste image URL / Ctrl+V" value="${h.logo}"
                            oninput="UI._toggleFetchBtn('fetch-h-logo',this.value)"
                            onpaste="UI.handleHeaderLogoPaste(event)">
                        <button type="button" id="fetch-h-logo" class="fetch-url-btn" ${UI._isValidUrl(h.logo) ? '' : 'disabled'}
                            onclick="UI.fetchHeaderLogoFromUrl()" title="Fetch & embed as base64">
                ...
                <div class="ctrl" id="h-logo-link-row" style="${h.logo ? '' : 'display:none;'}">
                    <label>Link URL (optional)</label>
                    <input type="text" id="h-logo-link" value="${h.logoLink || ''}" placeholder="https://..." oninput="State.updateHeader({ logoLink: this.value }); Preview.render();">
                </div>
                ...
                ${Blocks.ctrlLW(h.lw, 'UI.updLWHeader(0', 'hdr')}
        ...
        if (enabledEl) enabledEl.addEventListener('change', e => {
            ...
            if (wrap) {
                wrap.style.opacity = e.target.checked ? '1' : '0.4';
                wrap.style.pointerEvents = e.target.checked ? '' : 'none';
            }

            // Optional: If you need to specifically hide the LW controls when disabled
            const lwCtrl = document.querySelector('.lw-ctrl');
            if (lwCtrl) lwCtrl.style.display = e.target.checked ? '' : 'none';
    ...
    updLWHeader(idx, prop, val) {
        // 1. Get current state safely
        const header = State.get().header || {};
        const currentLw = header.lw || {};

        // 2. Create updated lw object
        const newLw = {
            ...currentLw,
            [prop]: (prop === 'enabled') ? (val === true || val === 'true') : val
        };

        // 3. Update State
        State.updateHeader({ lw: newLw });

        // 4. Re-build the panel HTML so the controls appear/disappear
        this.buildHeaderPanel();
    ...
    browseLibForLogo() {
        ...
        if (!manifest) { Utils.showToast('Manifest not loaded yet', 'warning'); return; }
        SocialIconPicker.openForImage((data) => {
            this._applyHeaderLogo(data);
            this.buildHeaderPanel();
        }, manifest);
    },


    // ── URL validation + fetch helpers ───────────────────────────
    _isValidUrl(url) {
    ...
    _toggleFetchBtn(btnId, urlVal) {
        ...
        if (btn) btn.disabled = !this._isValidUrl(urlVal);
    },

    _fetchUrlAsBase64(url) {
    ...
    fetchHeaderLogoFromUrl() {
        const url = (document.getElementById('h-logo-url')?.value || '').trim();
        if (!this._isValidUrl(url)) { Utils.showToast('Enter a valid URL (http/https/ftp)', 'warning'); return; }
        Utils.showToast('Fetching…', 'info');
        this._fetchUrlAsBase64(url)
            .then(data => { this._applyHeaderLogo(data); Utils.showToast('Logo embedded ✓', 'success'); })
    ...
    clearHeaderLogo() {
        // Clear both logo and logoLink
        State.updateHeader({ logo: '', logoLink: '' });
        this.buildHeaderPanel();
        Preview.render();
    },
    ...
    buildFooterPanel() {
        ...
        const PLATFORMS = [
            'afaqyLogo', 'facebook', 'x', 'twitter', 'instagram', 'linkedin', 'youtube',
        ...
        const iconRows = icons.map((ic, idx) => `
            <div class="soc-row" id="soc-row-${idx}">
                ...
                <div class="soc-icon-row">
                    <button type="button" class="soc-browse-thumb" onclick="UI.openSocialIconPicker(${idx})" title="Browse icon library">
                        ${ic.customSrc
                ? `<img src="${ic.customSrc}" class="soc-icon-preview" style="pointer-events:none;">`
                : `<div class="soc-icon-preview soc-icon-placeholder"><i class="fas fa-th-large" style="pointer-events:none;"></i></div>`}
                        <span class="soc-browse-hint">Browse</span>
                    </button>
                    <div class="soc-icon-actions">
                        ...
                        <input type="text" id="soc-url-inp-${idx}" class="soc-icon-url-inp" value="${ic.customSrc || ''}" placeholder="Or paste URL / path…"
                            oninput="UI.updSocial(${idx},'customSrc',this.value);UI._toggleFetchBtn('fetch-soc-${idx}',this.value)" style="flex:1;min-width:0;">
                        <button type="button" id="fetch-soc-${idx}" class="fetch-url-btn" ${UI._isValidUrl(ic.customSrc) ? '' : 'disabled'}
                            onclick="UI.fetchSocialIconFromUrl(${idx})" title="Fetch & embed as base64">
                            <i class="fas fa-download"></i>
                        </button>
                        ${ic.customSrc ? `<button type="button" class="soc-icon-clear" onclick="UI.updSocial(${idx},'customSrc','')" title="Clear — use default"><i class="fas fa-times"></i></button>` : ''}
                    </div>
                </div>
            </div>`).join('');
        ...
        el.innerHTML = `
            ...
            <div id="f-settings" style="${!enabled ? 'opacity:0.4;pointer-events:none;' : ''}">
                ...
                <div id="f-soc-section" style="${!socOn ? 'opacity:0.45;pointer-events:none;' : ''}">
                    ...
                    <div class="ctrl">
                        ...
                        <div class="btn-group-sm" style="flex-wrap:wrap;gap:4px;">
                            <button type="button" class="${(f.socialIconStyle || 'colored-compat') === 'colored-compat' ? 'active' : ''}"
                                onclick="UI.setSocialIconStyle('colored-compat')" title="PNG — brand-color circle with white icon. Gmail-safe. ✓ Default">
                                <i class="fas fa-circle" style="color:var(--accent)"></i> Gmail Colored
                            </button>
                            <button type="button" class="${(f.socialIconStyle || 'colored-compat') === 'white-compat' ? 'active' : ''}"
                                onclick="UI.setSocialIconStyle('white-compat')" title="PNG — white circle with brand-color icon. Gmail-safe.">
                                <i class="fas fa-circle" style="color:#fff"></i> Gmail White
                            </button>
                            <button type="button" class="${(f.socialIconStyle || 'colored-compat') === 'colored' ? 'active' : ''}"
                                onclick="UI.setSocialIconStyle('colored')" title="SVG — brand color, no background. Sharp at any size. Not Gmail-safe.">
                                <i class="fas fa-palette"></i> SVG Colored
                            </button>
                            <button type="button" class="${(f.socialIconStyle || 'colored-compat') === 'white' ? 'active' : ''}"
                                onclick="UI.setSocialIconStyle('white')" title="SVG — white icon, no background. Sharp at any size. Not Gmail-safe.">
                                <i class="fas fa-star"></i> SVG White
                            </button>
                        </div>
                    </div>
                    <div class="ctrl">
                ...
                ${Blocks.ctrlLW(f.lw, 'UI.updLWFooter(0', 'ftr')}
                <div class="row2">
    ...
    // ── Icon folder browser ───────────────────────────────────────
    openSocialIconPicker(idx) {
        const manifest = window._sIconsManifest;
        if (!manifest) { Utils.showToast('Manifest not loaded yet — try again in a moment', 'warning'); return; }
        SocialIconPicker.openForSocial(idx, manifest);
    },

    fetchSocialIconFromUrl(idx) {
        const url = (document.getElementById('soc-url-inp-' + idx)?.value || '').trim();
        if (!this._isValidUrl(url)) { Utils.showToast('Enter a valid URL (http/https/ftp)', 'warning'); return; }
        Utils.showToast('Fetching…', 'info');
        this._fetchUrlAsBase64(url)
            .then(data => {
                this.updSocial(idx, 'customSrc', data);
                this.buildFooterPanel();
                Utils.showToast('Icon embedded ✓', 'success');
            })
            .catch(e => Utils.showToast('Fetch failed: ' + e.message, 'error'));
    },
    ...
    updLWFooter(idx, prop, val) {
        // 1. Get current state safely
        const footer = State.get().footer || {};
        const currentLw = footer.lw || {};
        ...
        // 3. Update State
        State.updateFooter({ lw: newLw });

        // 4. Re-build panel & Preview
        this.buildFooterPanel();
        Preview.render();
    },
    ...
    loadTemplate(key, isUser) {
        ...
        if (!tpl) return;
        if (!confirm('Load "' + tpl.name + '"? Your current work will be replaced.')) return;
        State.applyTemplate(tpl);
        this.rebuildAll();
        Preview.render();
        Utils.showToast('Loaded: ' + tpl.name, 'success');
