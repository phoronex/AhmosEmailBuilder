...
const Export = {
    save() {
        ...
        overlay.innerHTML = `
        <div style="background:#fff;border-radius:14px;padding:28px 28px 22px;width:100%;max-width:420px;box-shadow:0 20px 60px rgba(0,0,0,0.25);font-family:inherit;">
                ...
                <div>
                    ...
                    <div style="margin-top:6px;display:flex;gap:4px;flex-wrap:wrap;">
                        ${['#2563eb','#059669','#7c3aed','#dc2626','#d97706','#0f172a','#0891b2','#be185d']
                            .map(c => `<div onclick="document.getElementById('sm-color').value='${c}';document.getElementById('sm-color-pick').value='${c}';"
        ...
        document.getElementById('sm-save').addEventListener('click', () => {
            ...
            // Persist meta in state so re-opening the dialog pre-fills correctly
            ...
            try {
                ...
                if (data.state && data.state.global) {
                    ...
                    State.data._meta = {
                        ...
                        icon:       data.icon       || State.data._meta?.icon || '',
                        thumbColor: data.thumbColor || State.data._meta?.thumbColor || ''
                    };
                    State.save();
                    UI.rebuildAll();
                    Preview.render();
